...
class _LoginScreenState extends State<LoginScreen> {
      ...
      Center(child: LayoutBuilder(
        builder: (context, constraints) => Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            ElevatedButton(
              onPressed: _login, 
              style: ElevatedButton.styleFrom(backgroundColor: botones, padding: EdgeInsets.symmetric(horizontal: 95, vertical: 15)),
              child: Text('Iniciar Sesión', style: TextStyle(color: Colors.white))
            ),
            SizedBox(height: 10),
            Text('Mi primer examen, ¿estará sencillo?', style: TextStyle(color: Colors.white))
          ])
        )
      ))
    ])
  ...
  Widget _buildTextField(String label, TextEditingController controller, {bool obscureText = false}) => TextField(
  ...
  void _login() {
    setState(() {
      ...
      if (error.isEmpty) Navigator.pushReplacementNamed(context, '/home');
    });
  }
}
